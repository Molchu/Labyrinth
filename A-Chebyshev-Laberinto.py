...
#---->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Función de heurística: Distancia de Chebyshev

#D = max(|x1 - x2|, |y1 - y2|)

def distancia_chebyshev(nodo1, nodo2):
    return max(abs(nodo1.x - nodo2.x), abs(nodo1.y - nodo2.y))
...
# Función para resolver el laberinto utilizando A* con heurística de distancia de Chebyshev
def resolver_laberinto_astar_chebyshev(inicio, objetivo):
    queue = deque()
    queue.append((inicio, []))
    ...
    while queue:
        ...
        # Calcular la heurística (distancia de Chebyshev) desde el nodo actual al objetivo
        heuristica = distancia_chebyshev(nodo_actual, objetivo)
        ...
        queue = deque(sorted(queue, key=lambda x: distancia_chebyshev(x[0], objetivo)))

    # Si no se encuentra una solución, regresar None
    return None

# Resolver el laberinto utilizando A* con heurística de distancia de Chebyshev
solucion_astar_chebyshev = resolver_laberinto_astar_chebyshev(inicio, objetivo)

if solucion_astar_chebyshev:
    print("Se encontró una solución utilizando A* con heurística de distancia de Chebyshev:")
    for i in range(len(solucion_astar_chebyshev) - 1):
        nodo1 = solucion_astar_chebyshev[i]
        nodo2 = solucion_astar_chebyshev[i + 1]
        dibujar_linea(nodo1, nodo2)
else:
    print("No se encontró una solución utilizando A* con heurística de distancia de Chebyshev.")
