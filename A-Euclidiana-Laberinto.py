...
def distancia_euclidiana(nodo1, nodo2):
    return ((nodo1.x - nodo2.x) ** 2 + (nodo1.y - nodo2.y) ** 2) ** 0.5
...
def resolver_laberinto_astar(inicio, objetivo):
    queue = deque()
    queue.append((inicio, []))
    ...
    while queue:
        ...
        # Calcular la heurística (distancia euclidiana) desde el nodo actual al objetivo
        heuristica = distancia_euclidiana(nodo_actual, objetivo)
        ...
        queue = deque(sorted(queue, key=lambda x: distancia_euclidiana(x[0], objetivo)))

    # Si no se encuentra una solución, regresar None
    return None

# Resolver el laberinto utilizando A* con heurística
solucion_astar = resolver_laberinto_astar(inicio, objetivo)

if solucion_astar:
    print("Se encontró una solución utilizando A* con heurística:")
    for i in range(len(solucion_astar) - 1):
        nodo1 = solucion_astar[i]
        nodo2 = solucion_astar[i + 1]
        dibujar_linea(nodo1, nodo2)
else:
    print("No se encontró una solución utilizando A* con heurística.")
